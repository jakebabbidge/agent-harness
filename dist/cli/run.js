    ...
    try {
        if (isWorkflow) {
            ...
            if (result.status === "failed") {
                console.error(`[agent-harness] Workflow failed at node '${result.failedNodeId ?? "unknown"}'.`);
                process.exit(1);
            }
            else {
                console.log(`[agent-harness] Workflow completed successfully. Nodes executed: ${result.nodeResults.length}`);
                process.exit(0);
            }
        }
        else {
            ...
            const truncated = result.resultText.length > 200
