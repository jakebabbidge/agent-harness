 ...
 */
export async function createWorktree(repoPath, taskId, baseBranch, tracker) {
    const branchName = `agent-harness/task-${taskId}`;
    const worktreePath = path.join(repoPath, '.worktrees', taskId);
    await runGit(['worktree', 'add', '-b', branchName, worktreePath, baseBranch], repoPath);
    const info = {
        taskId,
        worktreePath,
        branchName,
 ...
 */
export async function removeWorktree(repoPath, taskId, tracker) {
    const branchName = `agent-harness/task-${taskId}`;
    const worktreePath = path.join(repoPath, '.worktrees', taskId);
